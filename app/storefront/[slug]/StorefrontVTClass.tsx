...
export function StorefrontVTClass({ vehicles, sf, storeSlug, primaryColor, secondaryColor, whatsappPhone }: Props) {
  ...
  const gridClass = { '2': 'sm:grid-cols-2', '3': 'sm:grid-cols-2 lg:grid-cols-3', '4': 'sm:grid-cols-3 lg:grid-cols-4' }[sf.grid_cols]
  ...
  return (
    <FavoritesProvider storeSlug={storeSlug}>
      ...
      <div className="mb-4">
        <div className="flex gap-0">
          ...
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full h-10 pl-9 pr-8 border border-gray-300 bg-white text-sm focus:outline-none focus:border-gray-500"
            />
            {query && (
          ...
          {/* Buscar button — cor primária, sem radius */}
          <button
          ...
          {(sf.filter_brand || sf.filter_fuel || sf.filter_transmission || sf.filter_price) && (
            <button
              ...
              className={`flex-shrink-0 flex items-center gap-1.5 h-10 px-4 border-l-0 border text-sm font-medium transition-colors ${filtersOpen || activeFilterCount > 0 ? 'bg-gray-700 text-white border-gray-700' : 'bg-white text-gray-600 border-gray-300 hover:bg-gray-50'}`}
            >
              ...
              Filtros
              {activeFilterCount > 0 && (
                <span className="bg-white text-gray-900 text-xs w-4 h-4 flex items-center justify-center font-bold leading-none">
                  {activeFilterCount}
                </span>
              )}
            </button>
        ...
        {/* Filter panel */}
        {filtersOpen && (
          <div className="bg-white border border-t-0 border-gray-300 p-3 flex flex-wrap gap-2">
            {sf.filter_brand && brands.length > 1 && (
              <select value={brandFilter} onChange={e => setBrandFilter(e.target.value)}
                className="h-9 px-3 border border-gray-300 text-sm bg-white focus:outline-none cursor-pointer">
            ...
            {sf.filter_price && (
              <>
                <input type="number" value={minPrice} onChange={e => setMinPrice(e.target.value)}
                  placeholder="Preço mín." className="h-9 px-3 border border-gray-300 text-sm focus:outline-none w-32 bg-white" />
                <input type="number" value={maxPrice} onChange={e => setMaxPrice(e.target.value)}
                  placeholder="Preço máx." className="h-9 px-3 border border-gray-300 text-sm focus:outline-none w-32 bg-white" />
              </>
            )}
            {hasActiveFilters && (
      ...
      <div className="flex items-center justify-between mb-3 gap-3 flex-wrap">
        ...
        <select
          ...
          className="h-9 px-3 border border-gray-300 text-xs text-gray-600 bg-white focus:outline-none cursor-pointer"
        >
      ...
      {filtered.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-14 h-14 text-gray-400 mb-4">
            <path strokeLinecap="round" strokeLinejoin="round" d="m21 21-5.197-5.197m0 0A7.5 7.5 0 1 0 5.196 5.196a7.5 7.5 0 0 0 10.607 10.607Z" />
          </svg>
          <p className="text-gray-600 text-sm uppercase tracking-wide">Nenhum veículo encontrado</p>
          {hasActiveFilters && (
        ...
        <div className={`grid grid-cols-2 gap-2 sm:gap-3 ${gridClass}`}>
          {filtered.map((vehicle, index) => {
            ...
            // Specs row: Ano · Km · Comb
            const specParts: string[] = []
            if (sf.show_year && vehicle.year_model) specParts.push(`Ano: ${vehicle.year_model}`)
            if (sf.show_mileage && vehicle.mileage != null) specParts.push(`Km: ${formatMileage(vehicle.mileage)}`)
            if (sf.show_fuel && vehicle.fuel) specParts.push(`Comb: ${fuelLabel[vehicle.fuel] ?? vehicle.fuel}`)
              ...
              <AnimatedCard key={vehicle.id} delay={index * 40}>
                <Link href={vehicleHref} className="group block bg-white overflow-hidden hover:opacity-95 transition-opacity h-full">
                  ...
                  <div className="relative aspect-[4/3] bg-gray-200 overflow-hidden">
                    ...
                    {/* Marca tag — barra dourada na borda inferior da imagem */}
                    <div
