...
function Avatar({ name, size = 'md' }: { name: string; size?: 'sm' | 'md' | 'lg' }) {
  ...
  const sizeClass = size === 'lg' ? 'w-12 h-12' : size === 'md' ? 'w-11 h-11' : 'w-9 h-9'
  const textClass = size === 'lg' ? 'text-base' : size === 'md' ? 'text-sm' : 'text-sm'
  return (
    <div className={`${sizeClass} rounded-full ${color} flex items-center justify-center flex-shrink-0`}>
      <span className={`${textClass} font-semibold text-white`}>{initials}</span>
    </div>
  )
}
...
export function ConversationsContent({ messages, leads }: { messages: Message[]; leads: Lead[] }) {
  ...
  const totalMessages = active?.messages.length ?? 0
  const visibleMessages = active ? active.messages.slice(-visibleCount) : []
  const hasMore = totalMessages > visibleMessages.length
  ...
  }, [visibleCount, active?.phone])

  useEffect(() => {
    if (!isLoadingMore) return
    const t = setTimeout(() => setIsLoadingMore(false), 220)
  ...
  const handleScroll = () => {
    const el = scrollRef.current
    if (!el || !active || !hasMore || isLoadingMore) return
    if (el.scrollTop < LOAD_MORE_THRESHOLD) {
      prevScrollHeightRef.current = el.scrollHeight
      setIsLoadingMore(true)
      if (loadMoreTimerRef.current) clearTimeout(loadMoreTimerRef.current)
      loadMoreTimerRef.current = setTimeout(() => {
        setVisibleCount(c => Math.min(c + VISIBLE_STEP, totalMessages))
      }, 150)
    }
  }

  return (
    <div className="flex h-[calc(100vh-104px)] border border-border rounded-xl overflow-hidden bg-card shadow-sm">
      ...
      <div className="w-[360px] flex-shrink-0 flex flex-col border-r border-border">
        ...
        <div className="px-4 pt-4 pb-3 border-b border-border">
          <h2 className="text-lg font-semibold text-slate-900 mb-3">Conversas</h2>
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2.5 text-sm bg-slate-100 rounded-lg outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-all"
            />
          </div>
        </div>
      ...
      {active ? (
        <div className="flex-1 flex flex-col min-w-0">
          ...
          {/* Header da conversa */}
          <div className="flex items-center gap-3 px-5 py-3 border-b border-border bg-white min-h-[72px]">
            <Avatar name={active.name ?? active.phone} size="lg" />
            <div className="min-w-0">
              <p className="text-base font-semibold text-slate-900 truncate">{active.name ?? active.phone}</p>
          ...
          <div
            ...
            className="flex-1 overflow-y-auto px-6 py-5 space-y-1.5"
          ...
          >
            {hasMore && (
              <div
                className="flex justify-center pt-1 pb-3 transition-opacity duration-200"
                style={{ opacity: isLoadingMore ? 1 : 0.65 }}
              >
                <span className="inline-flex items-center gap-1.5 text-[11px] text-slate-500 bg-white/85 px-3 py-1 rounded-full shadow-sm backdrop-blur-sm">
                  <Loader2 size={12} className={isLoadingMore ? 'animate-spin' : ''} />
                  {isLoadingMore ? 'Carregando mensagens...' : 'Role para cima para ver mais'}
                </span>
              </div>
            )}
            {visibleMessages.map((msg, i) => {
              const isUser = msg.role === 'user'
              const showDate = i === 0 || new Date(msg.created_at).toDateString() !== new Date(visibleMessages[i - 1].created_at).toDateString()
              return (
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-4 text-center bg-slate-50">
          ...
          <div>
            <p className="text-base font-medium text-slate-600">Selecione uma conversa</p>
            <p className="text-sm text-muted-foreground mt-1">Clique em um contato para ver o histórico</p>
          </div>
        </div>
      )}
    </div>
